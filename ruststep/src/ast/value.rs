...
impl<'de, 'value> de::Deserializer<'de> for &'value RValue {
    ...
    {
        match self {
            RValue::Entity(id) => visitor.visit_map(SingleMapDeserializer::new("Entity", *id)),
            RValue::Value(id) => visitor.visit_map(SingleMapDeserializer::new("Value", *id)),
            RValue::ConstantEntity(name) => {
                visitor.visit_map(SingleMapDeserializer::new("ConstantEntity", name.clone()))
            }
            RValue::ConstantValue(name) => {
                visitor.visit_map(SingleMapDeserializer::new("ConstantValue", name.clone()))
            }
        }
    }
